...
export default function SettingsView() {
  ...
  return (
    <div style={{ maxWidth: isMobile ? "100%" : 640 }}>
      <h2 style={{ margin: "0 0 20px", fontSize: isMobile ? 18 : 20, fontWeight: 700 }}>Settings</h2>
      ...
      {tab === "habits" && (
        <div>
          {/* Add habit form */}
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 20, padding: 16, background: "var(--surface)", borderRadius: 10, border: "1px solid var(--border)" }}>
            <input
              value={newHabitName}
              onChange={(e) => setNewHabitName(e.target.value)}
              placeholder="New habit name..."
              style={inputStyle}
              onKeyDown={(e) => e.key === "Enter" && document.getElementById("add-habit-btn")?.click()}
            />
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {habitsByCategory.map((cat) => (
              <div key={cat.name} style={{ background: "var(--surface)", borderRadius: 10, border: "1px solid var(--border)", overflow: "hidden" }}>
                ...
                {cat.habits.length === 0 && <div style={{ padding: "12px 16px", color: "var(--text-muted)", fontSize: 13 }}>No habits in this category</div>}
                {cat.habits.map((habit) => (
                  <div key={habit._id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "0 12px 0 16px", borderBottom: "1px solid var(--border)", minHeight: 52 }}>
        ...
        <div>
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") {
                    ...
                    createTask({ title: newTaskTitle.trim(), date: today, isRecurring: true });
                    setNewTaskTitle("");
                  }
                }}
              />
          ...
          <div style={{ background: "var(--surface)", borderRadius: 10, border: "1px solid var(--border)", overflow: "hidden" }}>
            ...
            {(recurringTasks ?? []).length === 0 && (
              <div style={{ padding: "20px 16px", color: "var(--text-muted)", fontSize: 13, textAlign: "center" }}>No recurring tasks yet</div>
            )}
            {(recurringTasks ?? []).map((task) => (
              <div key={task._id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "0 8px 0 16px", borderBottom: "1px solid var(--border)", minHeight: 52 }}>
